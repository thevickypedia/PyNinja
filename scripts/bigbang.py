...
async def upload_large_file(
    ...
    async with aiohttp.ClientSession() as session:
        with (
            ...
            tqdm(
                total=total_chunks, unit="chunk", desc=f"Uploading {filename}"
            ) as pbar,
        ):
...
async def download_large_file(
    filepath: str = None, directory: str = None, destination: str = None
):
    ...
    if filepath:
        destination = os.path.join(
            destination or pathlib.Path(__file__).parent, os.path.basename(filepath)
        )
    else:
        destination = os.path.join(
